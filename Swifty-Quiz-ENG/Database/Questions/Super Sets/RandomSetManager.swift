...
class RandomSetManager {
	...
	// То есть при старте программы, все эти основные массивы наполняются, и освобождаются только при выходе из приложения
	// Пустыми они быть не могут, но главное - чтобы отработала showAllQuestionsNumber() при старте
